...
"""
        try:
            ...
            lines.append("-" * 80)
            lines.append(f"ℹ️  Aliases updated in {env_path}. Run 'source {env_path}' to apply.")
        except Exception:
            pass
    ...
    def search_run(self, project_id: Optional[str] = None, query: Optional[str] = None, limit: int = 20):
        ...
        for r in hits:
            ...
            print(f"  {r.relevance:.2f} | {source_tag}")
            print(f"       > {snippet}")
            print("-" * 80)
    ...
    def get_project_tree(self, project_id: str):
        ...
        for root, dirs, files in os.walk(proj['path']):
            dirnames[:] = [d for d in dirnames if d not in self.gatherer.IGNORE_DIRS]
